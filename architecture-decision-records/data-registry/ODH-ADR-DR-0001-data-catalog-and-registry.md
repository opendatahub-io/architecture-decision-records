# ODH-ADR-0001: Data Catalog and Data Registry for RHOAI
...
| **Table metadata** (get/update schema, properties) | ✅ | Schema via inference; properties via tags |
| **List tables** | ✅ | Filtered by SSAR per-item |
...
This is an intentional scope constraint, not a gap. The Non-Goals section explicitly states: "Full Iceberg REST compliance in Phase 1" is out of scope. The 6-of-13 endpoint groups that are fully supported cover the catalog use case; the 5 that are excluded are either engine-level operations that only make sense with a native Iceberg backend or have no customer need in Phase 1 (Views). The remaining 2 have partial support sufficient for Phase 1.
...
- **No purpose-built RBAC UI** — Admins manage catalog permissions via `oc create rolebinding` or the generic OCP Console RBAC views, which are not tailored for catalog-specific permission management.
- **No metadata sensitivity classification** — The catalog does not classify assets by sensitivity level (e.g., PII, confidential, internal). No existing RHAI registry (MLflow model registry, Feast feature store) implements data classification today. This is deferred to Phase 2 alongside lineage and governance work.
...
See [ADR 0002: Data Catalog API RBAC](internal-0002-catalog-api-rbac.md) for the full SSAR design, POC validation results, and per-resource granularity details.
...
### Catalog Server Provisioning

The catalog server is provisioned via the existing FeatureStore CRD, extended with a `spec.catalog.enabled` field. When enabled, the Feast operator deploys a separate catalog server pod alongside the feature store server, both pointing to the same PostgreSQL database. No new CRs are introduced.
...
1. **Export:** Read all catalog metadata from the Feast registry via the existing Iceberg REST Catalog endpoints. Since the Data Hub UI and all consumers use Iceberg REST, the export format is the Iceberg REST API response model (namespaces, tables, schemas, partitions).
2. **Transform:** Map Feast registry objects to native Iceberg catalog entries. The Phase 1 scope is already constrained to the subset that translates cleanly (see Non-Goals: "Full Iceberg REST compliance"), so this mapping should be lossless for Phase 1 data.
...
Build a new catalog server on UC OSS with a new operator, new image builds, and the existing RHOAI Catalogs & Registries UI components as a frontend foundation.
...
| Technical debt | 🟢 Low | Purpose-built for catalog |
| Operational complexity | 🔴 High | New operator, PostgreSQL, CRD |
| New infrastructure | 🔴 High | New pod, operator, CRD |

The Phase 2 POC revealed UC integration requires: a new operator (server deployment, PostgreSQL lifecycle, `UnityCatalog` CRD, SCIM sync controller, OAuthClient lifecycle, catalog auto-provisioning), an auth bridge (UC has no auth plugin architecture — requires a code fork with `AuthDecorator.java` patch), and a search API build. The architecture is sound and offers the highest long-term flexibility, but front-loads infrastructure work that can be deferred until UC matures.
...
#### Alternative 2: Extend Feast into a General Catalog

Widen Feast itself into a broader data catalog by extending the UI and adding new resource types to the registry.
...
| Upstream risk | 🟡 Moderate | Upstream Feast community may not adopt catalog extensions |
...
#### Alternative 3: PyIceberg-Based Standalone Catalog Server
...
See [Option 4: PyIceberg-Based Data Catalog Server](../option4-pyiceberg-catalog.md).
...
| Dimension | Rating | Notes |
|-----------|--------|-------|
| Time to MVP | 🔴 Slow | No data asset primitives to translate — greenfield catalog store required |
...
| Technical debt | 🔴 High | Hosting a data catalog inside a model lifecycle platform — architectural mismatch |
| Operational complexity | 🟢 Low | Existing MLflow deployment |
| New infrastructure | 🟢 None | Same MLflow process |

**Rejected.** MLflow is a model lifecycle platform (experiments, runs, models), not a data asset registry (tables, schemas, data sources). MLflow is expanding its registry concept to cover Models (already shipped), MCPs, Skills, and Agents, but these registries are shallow — name, version, and tags — without the metadata depth, schema management, or multi-level namespace hierarchy required for a Data Registry. Unlike Feast, where DataSource/SavedDataset map (with translation) to Iceberg Tables, MLflow has no source objects to translate from. Building Iceberg REST endpoints on MLflow would require a greenfield catalog data model hosted inside the MLflow process — effectively Option 1 (Greenfield) deployed differently, without any of the reuse benefits. The MLflow UI also has no data catalog components (~30 Feast UI files for data sources, search, and entity views have no MLflow equivalent). MLflow is valuable in the RHOAI data strategy as a **peer service** — Model Registry, model lineage, LLM tracing — not as the catalog backend.

See [MLflow as Catalog Backend Assessment](../mlflow-as-catalog-backend-assessment.md).
...
Kubernetes audit logging captures every SSAR call with user identity, resource, verb, namespace, and decision. This provides an audit trail for all catalog access without additional logging infrastructure.

### Encryption

Encryption in transit (TLS) and at rest (etcd encryption) are provided by the OpenShift platform and apply to all Data Catalog traffic and stored metadata. No additional encryption infrastructure is introduced.
...
The catalog server runs as a separate pod from the feature store server, providing process-level isolation. A catalog crash does not affect feature serving, and vice versa. Both servers share the same PostgreSQL database — a database outage affects both, but this is the same shared-fate model as any multi-service database deployment.

**Isolation measures:**
- **Process isolation:** Separate pods ensure catalog and feature store failures are independent.
- **Request timeouts:** Catalog endpoints enforce a request timeout (default: 30s) at the router level.
...
- **Restart coverage:** The catalog server runs as a Kubernetes Deployment with replica scaling and liveness/readiness probes.
...
|--------|---------------|---------|
| **Metrics** | Prometheus `/metrics` endpoint (already exposed by Feast) | Add `datacatalog_request_total`, `datacatalog_request_duration_seconds`, and `datacatalog_ssar_latency_seconds` counters/histograms, labeled by endpoint, verb, namespace, and HTTP status |
| **Logging** | Structured JSON logs (existing Feast logging) | Log every SSAR decision (user, resource, verb, allowed/denied) at INFO level. Log catalog CRUD operations at INFO. Log errors at ERROR with request context |
...
- **Feature flag:** A `DATACATALOG_ENABLED` environment variable (default: `true`) controls whether the Iceberg REST Catalog endpoints are registered at startup. When `false`, the Feast server behaves identically to pre-Data Catalog releases. The SSAR middleware is also disabled.
- **Rollback path:** Setting `DATACATALOG_ENABLED=false` in the Feast server Deployment immediately disables all catalog functionality. No data migration is needed — the Feast registry continues to operate as before. Catalog metadata stored via the translation layer is Feast registry data and remains intact.
...
|----------|----------|-------|
| **CPU** | +50-100m per replica | Catalog endpoints are stateless JSON serialization over Feast registry calls. No compute-intensive operations. SSAR adds one API server round-trip per request (~2-5ms). |
...
| **PostgreSQL** | No additional DB | Feast registry already uses PostgreSQL. Catalog metadata is stored as Feast registry objects. |

**Scaling characteristics:** Catalog request volume is expected to be low compared to feature serving (catalog browse/search is human-driven or pipeline-triggered, not real-time inference). Throughput targets will be baselined against Feast registry server benchmarks and validated with load testing during implementation. The translation layer adds serialization overhead that needs measurement. Capacity planning (scaling with catalog size and user count) will be informed by Phase 1 deployment metrics; the PostgreSQL-backed approach is expected to handle initial scale based on Feast registry production data.

**Degradation signals:** If catalog request latency exceeds 200ms at P95 (excluding SSAR), investigate Feast registry query performance. If SSAR latency exceeds 100ms at P95, investigate API server load.
...
| Table list (per namespace) | < 200ms | Translates to Feast entity/feature view list |
...
The catalog API surface SLO will be based on the existing Feast Registry SLO, to be confirmed during implementation. Since the catalog endpoints share the Feast server process, the catalog inherits Feast's availability and reliability characteristics.
...
* [ADR 0002: Data Catalog API RBAC](internal-0002-catalog-api-rbac.md) — SSAR pattern design, POC validation, per-resource granularity
...
* [Iceberg REST Catalog Adversarial Review](../iceberg-rest-catalog/adversarial-review.md) — strengths, weaknesses, and risks
* [Iceberg REST Catalog Interoperability Analysis](../iceberg-rest-catalog/interoperability-analysis.md) — engine interop matrix, credential vending, RHOAI value proposition
...
* [Option 4: PyIceberg-Based Data Catalog Server](../option4-pyiceberg-catalog.md) — standalone catalog alternative analysis
