...
* Usage data is correlated with API keys by their name, a non-secret API-key correlation identifier.
...
Access logs used for showback can contain user identity information. Starting RHOAI 3.6, gathering user identity information is opt-in and only happens once enabled in `MaasTenantConfig`; it remains disabled by default. Enabling it requires the following controls to be in place, enforced by Loki/LokiStack itself rather than by MaaS-built mechanisms:

* **Retention**: identity-bearing logs are subject to a maximum retention period, set explicitly via LokiStack's per-tenant retention configuration (`limits.tenants.<tenant>.retention`, falling back to `limits.global.retention`) rather than relying on Loki's default. The configured period is the shortest one that still covers the showback/billing cycle.
* **Deletion and erasure**: deleting identity-bearing log data on request (e.g., in response to a data-subject erasure request, or once it falls outside the retention window) is handled through Loki's compactor delete API, which can purge log streams matching a label/line selector (e.g., `user=<id>`) within a time range. This is documented as an operational runbook rather than implemented as a MaaS feature.
* **Access auditing**: queries against Loki are already authenticated per-tenant through the LokiStack gateway's OpenShift OAuth integration, which logs authenticated access; no separate audit mechanism is built by MaaS.
* Access to Loki and to the derived showback data (via the proxy and the dashboards) should be scoped per tenant, consistent with the tenant isolation model established for MaaS.

Once usage is correlated with API keys (RHOAI 3.6), the same tenant-scoping constraints apply to that correlation.
...
| -------------------- | ------------ | ---- | --------- |
| Model-as-a-Service team | Arik Hadas    | TBD  | Yes       |
| Platform/Observability team | Kevin Howell, Liel Bondy   | TBD  | Yes       |

* **Model-as-a-Service**: owns the EnvoyFilter on the AI Gateway, the OTEL collector and per-user usage proxy deployed by MaaS, and implements the administrator dashboard and the personal usage dashboard that consume the resulting data.
* **Platform/Observability**: owns the OTEL collector and Loki configuration deployed by the RHOAI operator (as from 3.6).
