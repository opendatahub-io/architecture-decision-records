...
spec:
  ...
  - type: prometheus
    metadata:
      ...
      threshold: "{{.Threshold}}"
      #Example Query: sum(rate(http_requests_total{job="data-science-collector-collector"}[1m]))
...
- **minReplicaCount**: Minimum number of replicas (typically 1)
- **maxReplicaCount**: Maximum number of replicas (set based on resource constraints)
- **pollingInterval**: How often to query metrics (in seconds, e.g., 30)
